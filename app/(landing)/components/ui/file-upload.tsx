...
const handelFileChange = (selectedFile?: File) => {
...
const removeFile = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    return (
        ...
        onDrop={(e) => {
            e.preventDefault();
            handelFileChange(e.dataTransfer.files?.[0])
        }}
        className="flex flex-col justify-center items-center w-full py-6 border border-dashed border-primary bg-primary-light">
            <input type="file" className="hidden" ref={fileInputRef} accept="image/*" onChange={(e) => handelFileChange(e.target.files?.[0])} />
            {
                !file ? (
